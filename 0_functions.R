...
weibull_posterior_draw <- function(formula, data, 
                                   epsilon=100,
                                   eta_initial=NULL,
                                   n_mcmc = 4000,
                                   thin = 4,
  ...
  # get n_j beta parameters that accounts for weights
  nj = apply( rmb_out$events * rep.col(w,J), 2, sum)
  #
...
event_time_posterior_predictive_draws <- function(
  survival_probabilities,
  time_grid,
  censor_time=0,
  nobs = 100
){
  CDF = 1-survival_probabilities
  if(nrow(CDF)>nobs){
    CDF = CDF[sample(1:nrow(CDF),size = nobs,replace = F),]
  } else {
    CDF = CDF[sample(1:nrow(CDF),size = nobs,replace = T),]
  }
  
  CDF = cbind(0,CDF)
  CDF = cbind(CDF,1)
  time_grid_new = c(0,time_grid,max(time_grid)*1.1)
  
  if(censor_time==0){
    out = c()
    for(r in 1:nrow(CDF)){
      xout = runif(1)
      line_out = approx(x=CDF[r,],y=time_grid_new, xout, method = "linear")
      out = c(out,line_out$y)
    }
  } 
  if(censor_time>0){ # truncated distribution
    lower_bound = c()
    for(r in 1:nrow(CDF)){
      line_out = approx(x=time_grid_new,y=CDF[r,], censor_time, method = "linear")
      lower_bound = c(lower_bound,line_out$y)
    }
    out = c()
    for(r in 1:nrow(CDF)){
      xout = runif(1,lower_bound[r],1)
      line_out = approx(x=CDF[r,],y=time_grid_new, xout, method = "linear")
      out = c(out,line_out$y)
    }
  }
